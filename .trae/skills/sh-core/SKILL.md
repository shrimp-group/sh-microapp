...
com.wkclz.core
├── annotation/
│   ├── FieldDesc          # 字段描述注解（value + notNull），替代已废弃的@Desc
│   ├── Desc               # [已废弃] 描述注解，被@FieldDesc替代
│   ├── ApiDesc            # API描述注解
...
- `@FieldDesc("描述")` / `@FieldDesc(value="描述", notNull=true)` — 标注字段含义和非空约束，用于SQL Provider自动校验
- `@ApiDesc("描述")` — 标注API接口描述
