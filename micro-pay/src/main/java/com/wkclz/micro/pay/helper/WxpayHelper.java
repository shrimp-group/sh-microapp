...
public class WxpayHelper {
    ...
    public PayOrderDto pay(PayOrder payOrder, HttpServletRequest req, HttpServletResponse rep) {
        String openId = IdentityContext.getAttribute("open-id");
    ...
    public PayOrder payNotify(PayOrder payOrder, WxPayNotifyV3Result.DecryptNotifyResult notifyResult) {
        ...
        */
        ...
        String successTime = notifyResult.getSuccessTime().replaceAll("([+-]\\d\\d):\\d\\d$", "$100");
        ...
        try {
            ...
            log.info("微信支付成功，订单状态已更新为 PAID, orderNo: {}", payOrder.getOrderNo());
        } catch (Exception e) {
            log.error("微信支付成功，更新订单状态失败, orderNo: {}", payOrder.getOrderNo(), e);
        }
    ...
    */
    ...
    // 发起退款
    public String wxTradeRefund(PayOrder payOrder, BigDecimal refundAmount, String refundNo, String reason) {
        if (reason == null) {
            reason = "发起退款";
        }
        ...
        request.setTransactionId(payFlowNo);
        request.setOutTradeNo(payOrder.getOutTradeNo());
        // outRefundNo 使用传入的 refundNo（部分退款幂等键），为空时回退到 orderNo（全单退款，向后兼容）
        request.setOutRefundNo((refundNo != null && !refundNo.isBlank()) ? refundNo : payOrder.getOrderNo());

        WxPayRefundV3Request.Amount amount = new WxPayRefundV3Request.Amount();
        // 金额单位为分
        amount.setTotal(paymentAmount.multiply(new BigDecimal("100")).intValue());
        amount.setCurrency("CNY");
        // refundAmount 为空表示全单退款，使用 total；非空时按本次退款金额转换（元 → 分）
    ...
    public String wxRefundNotify(WxPayRefundNotifyV3Result refundNotifyResult) {
        ...
        if ("SUCCESS".equals(returnCode)) {
            ...
            */
            ...
            // 通过 SPI 更新订单退款状态
            try {
                payOrderSpi.updateOrderToRefunded(payOrder.getOrderNo());
                log.info("微信退款成功，订单状态已更新为 REFUNDED, orderNo: {}", payOrder.getOrderNo());
            } catch (Exception e) {
                log.error("微信退款成功，更新订单状态失败, orderNo: {}", payOrder.getOrderNo(), e);
            }
