 ...
 * @author shrimp
 * @table mdm_file_bucket (Bucket管理) 重新生成代码会覆盖
 */

@Data
@EqualsAndHashCode(callSuper = false)
public class MdmFileBucket extends BaseEntity {
    ...
    public static MdmFileBucket copy(MdmFileBucket source, MdmFileBucket target) {
        if (target == null ) { target = new MdmFileBucket();}
    ...
    public static MdmFileBucket copyIfNotNull(MdmFileBucket source, MdmFileBucket target) {
        if (target == null ) { target = new MdmFileBucket();}
        if (source == null) { return target; }
