...
其他模块注入 `MsgApi` 即可发送消息，发送人自动从 `SessionHelper.getUserCode()` 获取：
...
- IAM SDK（`SessionHelper.getUserCode()` 获取当前用户）
...
| 依赖 | 用途 |
|------|------|
| `com.wkclz.iam:iam-sdk` | SessionHelper 获取当前用户 userCode |
...
| 发送消息时 userCode 为空 | `SessionHelper.getUserCode()` 依赖 IAM 上下文，确保请求携带有效 Token |
