...
public class FileRecordRest {
    ...
    /**
     * @api {get} /micro-file/record/page 6. 附件-获取分页
     * @apiGroup FILE
     *
     * @apiVersion 0.0.1
    ...
    public R mdmFileRecordPage(MdmFileRecord entity) {
        Assert.notNull(entity.getTimeFrom(), "timeFrom can not be null");
        Assert.notNull(entity.getTimeTo(), "timeTo can not be null");
        PageData<MdmFileRecord> page = mdmFileRecordService.getPage(entity);
        if (CollectionUtils.isEmpty(page.getRecords())) {
            return R.ok(page);
        }
        List<MdmFileRecordDto> rows = page.getRecords().stream().map(MdmFileRecordDto::copy).collect(Collectors.toList());
        fileApi.sign(rows, MdmFileRecordDto::getFileId, MdmFileRecordDto::setPreviewUrl);

        // 转换成新 page
        PageData<MdmFileRecordDto> newPage = PageData.of(rows, page.getTotal(), page.getCurrent(), page.getSize());
        return R.ok(newPage);
    }

    /**
     * @api {get} /micro-file/record/info 7. 附件-获取详情
     * @apiGroup FILE
     *
     * @apiVersion 0.0.1
    ...
    public R mdmFileRecordInfo(MdmFileRecord entity) {
        Assert.notNull(entity.getId(), ResultCode.PARAM_NO_ID.getMessage());
        entity = mdmFileRecordService.getInfo(entity);
        MdmFileRecordDto dto = MdmFileRecordDto.copy(entity);
        dto.setPreviewUrl(fileApi.sign(entity));
        return R.ok(dto);
    }

    /**
     * @api {post} /micro-file/record/remove 8. 附件-删除
     * @apiGroup FILE
     *
     * @apiVersion 0.0.1
