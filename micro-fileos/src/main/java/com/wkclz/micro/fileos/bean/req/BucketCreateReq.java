package com.wkclz.micro.fileos.bean.req;

import io.swagger.v3.oas.annotations.media.Schema;
import jakarta.validation.constraints.NotBlank;
import lombok.Data;

import java.io.Serializable;

@Data
@Schema(description = "Bucket创建请求")
public class BucketCreateReq implements Serializable {

    @NotBlank(message = "Bucket名称不能为空")
    @Schema(description = "Bucket名称", requiredMode = Schema.RequiredMode.REQUIRED)
    private String bucketName;

    @NotBlank(message = "OSS服务商标识不能为空")
    @Schema(description = "OSS服务商标识", requiredMode = Schema.RequiredMode.REQUIRED)
    private String ossSp;

    @Schema(description = "内网Endpoint")
    private String endpointInner;

    @Schema(description = "外网Endpoint")
    private String endpointOuter;

    @Schema(description = "区域")
    private String region;

    @Schema(description = "Access Key")
    private String accessKey;

    @Schema(description = "Secret Key")
    private String secretKey;

    @Schema(description = "默认标识")
    private Integer defaultFlag;

    @Schema(description = "路径中的system段")
    private String system;

    @Schema(description = "备注")
    private String remark;
}
